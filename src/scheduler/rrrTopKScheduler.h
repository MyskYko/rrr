...
namespace rrr {
  ...
  public:
    // constructors
    TopKScheduler(std::vector<Ntk *> vNtks, Parameter const *pPar);
    ~TopKScheduler();
  ...
  template <typename Ntk, typename Opt, typename Par>
  template<typename... Args>
  inline void TopKScheduler<Ntk, Opt, Par>::Print(int nVerboseLevel, std::string prefix, Args... args) {
  ...
  template <typename Ntk, typename Opt, typename Par>
  inline seconds TopKScheduler<Ntk, Opt, Par>::GetRemainingTime() const {
    if(nTimeout == 0) {
      return 0;
    }
  ...
  template <typename Ntk, typename Opt, typename Par>
  inline double TopKScheduler<Ntk, Opt, Par>::GetElapsedTime() const {
    time_point timeCurrent = GetCurrentTime();
  ...
  template <typename Ntk, typename Opt, typename Par>
  template <typename T>
  void TopKScheduler<Ntk, Opt, Par>::AddToSummary(std::vector<std::string> &keys, std::map<std::string, T> &m, summary<T> const &result) const {
  ...
  template <typename Ntk, typename Opt, typename Par>
  TopKScheduler<Ntk, Opt, Par>::TopKScheduler(std::vector<Ntk *> vNtks, Parameter const *pPar) :
    vNtks(vNtks),
  ...
  template <typename Ntk, typename Opt, typename Par>
  TopKScheduler<Ntk, Opt, Par>::~TopKScheduler() {
  ...
  template <typename Ntk, typename Opt, typename Par>
  void TopKScheduler<Ntk, Opt, Par>::Run() {
    ...
    for(int round = 0; cost > 0; round++) {
      Print(0, "", "round", round, ":", "cost", "=", cost, "elapsed", "=", GetElapsedTime(), "s");
      // build queue
      for(int i = 0; i < int_size(vNtks); i++) {
      ...
      // set delta
      double tDelta = 1000000000; // some big value for fallback
      std::vector<int> vIndices(vNtks.size());
      for(int k = 0; k < K; k++) {
        int idx = -1;
        double tNext = std::numeric_limits<double>::max();
        for(int i = 0; i < int_size(vNtks); i++) {
          Print(1, "", "module", i, ":", "next", "=", vOpts[i]->GetNext());
          if(tNext > vOpts[i]->GetNext(vIndices[i])) { // TODO: this assumes we are increasing threshold
            tNext = vOpts[i]->GetNext(vIndices[i]);
            idx = i;
          }
        }
        if(idx == -1) {
        ...
        if(k == K - 1) {
          tDelta = tNext - vOpts[0]->GetCurrent();
        }
        vIndices[idx]++;
      }
      if(tDelta < 0) {
      ...
      for(int k = 0; k < K || tDelta == 0; k++) {
        ...
        if(vOpts[idx]->RemoveNext()) {
          nRemoved++;
        }
        nTemporary = vOpts[idx]->GetNumTemporary();
      }
      Print(0, "", "removed ", nRemoved, "out of", K);
      cost = 0;
      for(Ntk *pNtk: vNtks) {
