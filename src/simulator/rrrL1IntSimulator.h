...
namespace rrr {
  ...
  private:
    ...
    // backups
    std::vector<L1IntSimulator> vBackups;
  ...
  public:
    // constructors
    L1IntSimulator();
    L1IntSimulator(Parameter const *pPar);
  ...
  template <typename Ntk>
  inline void L1IntSimulator<Ntk>::Clear(int n, itr x) const {
    std::fill(x, x + n, 0);
  }

  template <typename Ntk>
  inline void L1IntSimulator<Ntk>::Fill(int n, itr x) const {
    std::fill(x, x + n, one);
  }
  
  template <typename Ntk>
  inline void L1IntSimulator<Ntk>::Copy(int n, itr dst, citr src, bool c) const {
    if(!c) {
  ...
  template <typename Ntk>
  inline void L1IntSimulator<Ntk>::And(int n, itr dst, citr src0, citr src1, bool c0, bool c1) const {
    if(!c0) {
      if(!c1) {
  ...
  template <typename Ntk>
  inline void L1IntSimulator<Ntk>::Or(int n, itr dst, citr src0, citr src1, bool c0, bool c1) const {
    if(!c0) {
      if(!c1) {
        for(int i = 0; i < n; i++, dst++, src0++, src1++) {
          *dst = *src0 | *src1;
  ...
  template <typename Ntk>
  inline void L1IntSimulator<Ntk>::Xor(int n, itr dst, citr src0, citr src1, bool c) const {
    if(!c) {
      for(int i = 0; i < n; i++, dst++, src0++, src1++) {
        *dst = *src0 ^ *src1;
      }
  ...
  template <typename Ntk>
  inline bool L1IntSimulator<Ntk>::IsZero(int n, citr x, word mask) const {
    if(mask == one) {
      for(int i = 0; i < n; i++, x++) {
        if(*x) {
          return false;
  ...
  template <typename Ntk>
  inline int L1IntSimulator<Ntk>::Popcount(int n, citr x, word mask) const {
    int count = 0;
    if(mask == one) {
      for(int i = 0; i < n; i++, x++) {
  ...
  template <typename Ntk>
  inline bool L1IntSimulator<Ntk>::IsEq(int n, citr x, citr y, bool c, word mask) const {
    if(!c) {
      if(mask == one) {
  ...
  template <typename Ntk>
  inline std::vector<std::vector<unsigned long long>> L1IntSimulator<Ntk>::ColumnWisePopcount(std::vector<citr> const &v) {
  ...
  template <typename Ntk>
  inline std::vector<int> L1IntSimulator<Ntk>::BinaryToIntegerUnsigned(std::vector<std::vector<word>> const &r) const {
    static constexpr int N = nPatterns;
  ...
  template <typename Ntk>
  inline std::vector<int> L1IntSimulator<Ntk>::BinaryToIntegerSigned(std::vector<std::vector<word>> const &r) const {
    static constexpr int N = nPatterns;
    std::vector<int> res(64 * N);
    for(int i = 0; i < N; i++) {
      for(int b = 0; b < 64; b++) {
  ...
  template <typename Ntk>
  inline void L1IntSimulator<Ntk>::BinarySubtract(std::vector<std::vector<word>> &a, std::vector<std::vector<word>> const &b) const {
    // overwrite a = a - b
    static constexpr int N = nPatterns;
    int L = a.size();
  ...
  template <typename Ntk>
  inline void L1IntSimulator<Ntk>::Print(int n, citr x) const {
  ...
  template <typename Ntk>
  void L1IntSimulator<Ntk>::ActionCallback(Action const &action) {
    switch(action.type) {
    case REMOVE_FANIN:
      assert(fInitialized);
      if(action.id == target) {
  ...
  template <typename Ntk>
  inline unsigned L1IntSimulator<Ntk>::StartTraversal(int n) {
    do {
      for(int i = 0; i < n; i++) {
        iTrav++;
        if(iTrav == 0) {
  ...
  template <typename Ntk>
  void L1IntSimulator<Ntk>::SimulateNode(Ntk *pNtk_, std::vector<word> &v, int id) const {
    itr x = v.end();
  ...
  template <typename Ntk>
  void L1IntSimulator<Ntk>::Simulate() {
  ...
  template <typename Ntk>
  void L1IntSimulator<Ntk>::Resimulate() {
    time_point timeStart = GetCurrentTime();
    if(nVerbose) {
  ...
  template <typename Ntk>
  void L1IntSimulator<Ntk>::ReadStimuli(Pattern *pPat) {
  ...
  template <typename Ntk>
  void L1IntSimulator<Ntk>::ComputeCare(int id) {
  ...
  template <typename Ntk>
  void L1IntSimulator<Ntk>::Initialize() {
    assert(fGenerated);
    vValues.resize(nWords * pNtk->GetNumNodes());
  ...
  template <typename Ntk>
  void L1IntSimulator<Ntk>::Save(int slot) {
  ...
  template <typename Ntk>
  void L1IntSimulator<Ntk>::Load(int slot) {
  ...
  template <typename Ntk>
  L1IntSimulator<Ntk>::L1IntSimulator() :
    pNtk(NULL),
    nVerbose(0),
    fSave(false),
    fGenerated(false),
  ...
  template <typename Ntk>
  L1IntSimulator<Ntk>::L1IntSimulator(Parameter const *pPar) :
    pNtk(NULL),
  ...
  template <typename Ntk>
  void L1IntSimulator<Ntk>::AssignNetwork(Ntk *pNtk_, bool fReuse) {
    ...
    sUpdates.clear();
    pNtk = pNtk_;
    pNtk->AddCallback(std::bind(&L1IntSimulator<Ntk>::ActionCallback, this, std::placeholders::_1));
    if(!fGenerated) {
  ...
  template <typename Ntk>
  int L1IntSimulator<Ntk>::CheckRedundancy(int id, int idx) {
    if(!fInitialized) {
      Initialize();
    }
    ComputeCare(id);
      ...
      pNtk->ForEachPoDriverIdx([&](int idx, int fi, bool c) {
        ...
        if(((idx + 1) % nPosPerClass) == 0) {
          if(nUpdatedPos != 0) {
            ...
            auto res = BinaryToIntegerSigned(rInc);
            for(int j = 0; j < nPatterns * 64 - nRemainder; j++) {
              nNew += std::abs(vDiffs[nClass][j] + res[j]);
            }
            vInc.clear();
            vDec.clear();
            nUpdatedPos = 0;
  ...
  template <typename Ntk>
  int L1IntSimulator<Ntk>::CheckFeasibility(int id, int fi, bool c) {
    assert(0);
    if(!fInitialized) {
      Initialize();
    }
  ...
  template <typename Ntk>
  int L1IntSimulator<Ntk>::GetDefaultThreshold() {
    return 0;
  }

  template <typename Ntk>
  int L1IntSimulator<Ntk>::GetCurrent() {
    if(!fInitialized) {
      Initialize();
    }
    if(!sUpdates.empty()) {
  ...
  template <typename Ntk>
  void L1IntSimulator<Ntk>::ResetSummary() {
    durationSimulation = 0;
    durationCare = 0;
  };
  
  template <typename Ntk>
  summary<int> L1IntSimulator<Ntk>::GetStatsSummary() const {
    summary<int> v;
    return v;
  };
  
  template <typename Ntk>
  summary<double> L1IntSimulator<Ntk>::GetTimesSummary() const {
    summary<double> v;
