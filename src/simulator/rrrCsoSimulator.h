...
namespace rrr {
  ...
  template <typename Ntk>
  inline bool CsoSimulator<Ntk>::AtMostK(int n, citr x, int k) const {
    int count = 0;
    for(int i = 0; i < n; i++, x++) {
  ...
  /*
  template <typename Ntk>
  void CsoSimulator<Ntk>::SimulateOneWordNode(std::vector<word> &v, int id, int offset, int to_negate) {
    itr x = v.end();
  ...
  */
  ...
  /*
  template <typename Ntk>
  void CsoSimulator<Ntk>::SimulatePartNode(std::vector<word> &v, int id, int n, int offset, int to_negate) {
    itr x = v.end();
    itr y = v.begin() + id * nStimuli + offset;
    bool cx = false;
    switch(pNtk->GetNodeType(id)) {
  ...
  */
  ...
  /*
  template <typename Ntk>
  void CsoSimulator<Ntk>::SimulateOneWord(int offset) {
    time_point timeStart = GetCurrentTime();
    if(nVerbose) {
  ...
  */
  ...
  template <typename Ntk>
  void CsoSimulator<Ntk>::GenerateRandomStimuli() {
    if(nVerbose) {
  ...
  /*
  template <typename Ntk>
  void CsoSimulator<Ntk>::ComputeCarePart(int nWords_, int offset) {
    if(nVerbose) {
  ...
  */
  ...
  template <typename Ntk>
  std::vector<unsigned long long> CsoSimulator<Ntk>::ComputeSdc(std::vector<int> const &ids) {
    if(!fInitialized) {
      Initialize();
    }
  ...
  /*
  template <typename Ntk>
  void CsoSimulator<Ntk>::AddCex(std::vector<VarValue> const &vCex) {
    if(nVerbose) {
      std::cout << "cex: ";
      for(VarValue c: vCex) {
