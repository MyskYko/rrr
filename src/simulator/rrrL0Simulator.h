...
namespace rrr {
  ...
  public:
    // constructors
    L0Simulator();
    L0Simulator(Parameter const *pPar);
  ...
  template <typename Ntk>
  inline void L0Simulator<Ntk>::Clear(int n, itr x) const {
    std::fill(x, x + n, 0);
  }

  template <typename Ntk>
  inline void L0Simulator<Ntk>::Fill(int n, itr x) const {
    std::fill(x, x + n, one);
  }
  
  template <typename Ntk>
  inline void L0Simulator<Ntk>::Copy(int n, itr dst, citr src, bool c) const {
    if(!c) {
  ...
  template <typename Ntk>
  inline void L0Simulator<Ntk>::And(int n, itr dst, citr src0, citr src1, bool c0, bool c1) const {
    if(!c0) {
      if(!c1) {
  ...
  template <typename Ntk>
  inline void L0Simulator<Ntk>::Or(int n, itr dst, citr src0, citr src1, bool c0, bool c1) const {
    if(!c0) {
      if(!c1) {
        for(int i = 0; i < n; i++, dst++, src0++, src1++) {
          *dst = *src0 | *src1;
  ...
  template <typename Ntk>
  inline void L0Simulator<Ntk>::Xor(int n, itr dst, citr src0, citr src1, bool c) const {
    if(!c) {
      for(int i = 0; i < n; i++, dst++, src0++, src1++) {
        *dst = *src0 ^ *src1;
      }
  ...
  template <typename Ntk>
  inline bool L0Simulator<Ntk>::IsZero(int n, citr x, word mask) const {
    if(mask == one) {
      for(int i = 0; i < n; i++, x++) {
        if(*x) {
          return false;
  ...
  template <typename Ntk>
  inline int L0Simulator<Ntk>::Popcount(int n, citr x, word mask) const {
    int count = 0;
    if(mask == one) {
      for(int i = 0; i < n; i++, x++) {
  ...
  template <typename Ntk>
  inline bool L0Simulator<Ntk>::IsEq(int n, citr x, citr y, bool c, word mask) const {
    if(!c) {
      if(mask == one) {
  ...
  template <typename Ntk>
  inline void L0Simulator<Ntk>::Print(int n, citr x) const {
  ...
  template <typename Ntk>
  void L0Simulator<Ntk>::ActionCallback(Action const &action) {
    switch(action.type) {
    case REMOVE_FANIN:
      assert(fInitialized);
      if(action.id == target) {
  ...
  template <typename Ntk>
  inline unsigned L0Simulator<Ntk>::StartTraversal(int n) {
    do {
      for(int i = 0; i < n; i++) {
        iTrav++;
        if(iTrav == 0) {
  ...
  template <typename Ntk>
  void L0Simulator<Ntk>::SimulateNode(Ntk *pNtk_, std::vector<word> &v, int id) const {
    itr x = v.end();
  ...
  template <typename Ntk>
  void L0Simulator<Ntk>::Resimulate() {
    time_point timeStart = GetCurrentTime();
    if(nVerbose) {
  ...
  template <typename Ntk>
  void L0Simulator<Ntk>::ReadStimuli(Pattern *pPat) {
  ...
  template <typename Ntk>
  void L0Simulator<Ntk>::Initialize() {
    assert(fGenerated);
    vValues.resize(nWords * pNtk->GetNumNodes());
  ...
  template <typename Ntk>
  void L0Simulator<Ntk>::Save(int slot) {
  ...
  template <typename Ntk>
  void L0Simulator<Ntk>::Load(int slot) {
  ...
  template <typename Ntk>
  L0Simulator<Ntk>::L0Simulator(Parameter const *pPar) :
    pNtk(NULL),
  ...
  template <typename Ntk>
  void L0Simulator<Ntk>::AssignNetwork(Ntk *pNtk_, bool fReuse) {
    ...
    sUpdates.clear();
    pNtk = pNtk_;
    pNtk->AddCallback(std::bind(&L0Simulator<Ntk>::ActionCallback, this, std::placeholders::_1));
    if(!fGenerated) {
  ...
  template <typename Ntk>
  int L0Simulator<Ntk>::CheckRedundancy(int id, int idx) {
    if(!fInitialized) {
      Initialize();
    }
    ComputeCare(id);
  ...
  template <typename Ntk>
  int L0Simulator<Ntk>::CheckFeasibility(int id, int fi, bool c) {
    assert(0);
    if(!fInitialized) {
      Initialize();
    }
  ...
  template <typename Ntk>
  void L0Simulator<Ntk>::ResetSummary() {
    durationSimulation = 0;
    durationCare = 0;
  };
  
  template <typename Ntk>
  summary<int> L0Simulator<Ntk>::GetStatsSummary() const {
    summary<int> v;
    return v;
  };
  
  template <typename Ntk>
  summary<double> L0Simulator<Ntk>::GetTimesSummary() const {
    summary<double> v;
