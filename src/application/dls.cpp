...
#include "scheduler/rrrDlsScheduler.h"
#include "optimizer/rrrDlsOptimizer.h"
#include "analyzer/rrrDlsAnalyzer.h"
#include "simulator/rrrDlsSimulator.h"
...
namespace rrr {
  ...
  void PerformDls(std::vector<Ntk *> &vNtks, Parameter const *pPar) {
    Pattern *pPat = NULL;
    if(!pPar->strPattern.empty()) {
      pPat = new Pattern;
      pPat->Read(pPar->strPattern, vNtks[0]->GetNumPis());
      if(!pPar->strPatternOutput.empty()) {
        pPat->ReadOutput(pPar->strPatternOutput, vNtks[0]->GetNumPos());
      }
    ...
    DlsScheduler<Ntk, DlsOptimizer<Ntk, DlsAnalyzer<Ntk, DlsSimulator<Ntk>>>, Partitioner<Ntk>> sch(vNtks, pPar);
    sch.Run();
    if(pPat) {
      delete pPat;
    }
