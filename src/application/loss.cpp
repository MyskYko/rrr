...
#include "scheduler/rrrRotatingThresholdScheduler.h"
...
namespace rrr {
  ...
  void PerformLoss(std::vector<Ntk *> &vNtks, Parameter const *pPar) {
    ...
    RotatingThresholdScheduler<Ntk, CsoOptimizer<Ntk, ThresholdAnalyzer<Ntk, LossSimulator<Ntk>, double, true>, double>, Partitioner<Ntk>> sch(vNtks, pPar);
    sch.Run();
    if(pPat) {
      delete pPat;
    }
