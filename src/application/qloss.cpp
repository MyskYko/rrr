...
#include "scheduler/rrrTopKScheduler.h"
#include "optimizer/rrrQueueOptimizer.h"
#include "analyzer/rrrThresholdAnalyzer.h"
#include "simulator/rrrLossSimulator.h"
...
namespace rrr {
  ...
  void PerformLoss(std::vector<Ntk *> &vNtks, Parameter const *pPar) {
    Pattern *pPat = NULL;
    ...
    TopKScheduler<Ntk, QueueOptimizer<Ntk, ThresholdAnalyzer<Ntk, LossSimulator<Ntk>, double, true>, double>, Partitioner<Ntk>> sch(vNtks, pPar);
    sch.Run();
    if(pPat) {
      delete pPat;
    }
...
int main(int argc, char **argv) {
  ...
  */
  ...
  rrr::PerformLoss(vNtks, &Par);
  
  if(!output_filename.empty()) {
    for(int i = 0; i < rrr::int_size(input_filenames); i++) {
      rrr::DumpAig(output_filename + "_" + std::to_string(i) + ".aig", vNtks[i]);
    }
  }
