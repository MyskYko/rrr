...
#include "simulator/rrrL1BiasSimulator.h"
...
namespace rrr {
  ...
  void PerformL1Bias(std::vector<Ntk *> &vNtks, Parameter const *pPar) {
    Pattern *pPat = NULL;
    ...
    RotatingThresholdScheduler<Ntk, CsoOptimizer<Ntk, ThresholdAnalyzer<Ntk, L1BiasSimulator<Ntk>, int, true>>, Partitioner<Ntk>> sch(vNtks, pPar);
    sch.Run();
    if(pPat) {
      delete pPat;
    }
