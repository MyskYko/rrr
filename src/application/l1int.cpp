...
#include "simulator/rrrL1IntSimulator.h"
...
namespace rrr {
  ...
  void PerformL1Int(Ntk *pNtk, Parameter const *pPar) {
    Pattern *pPat = NULL;
    ...
    CsoScheduler<Ntk, CsoOptimizer<Ntk, ThresholdAnalyzer<Ntk, L1IntSimulator<Ntk>, int, true>>, Partitioner<Ntk>> sch(pNtk, pPar);
    sch.Run();
    if(pPat) {
      delete pPat;
    }
