...
#include "simulator/rrrL0Simulator.h"
...
namespace rrr {
  ...
  void PerformL0(Ntk *pNtk, Parameter const *pPar) {
    Pattern *pPat = NULL;
    ...
    CsoScheduler<Ntk, CsoOptimizer<Ntk, ThresholdAnalyzer<Ntk, L0Simulator<Ntk>, int, true>>, Partitioner<Ntk>> sch(pNtk, pPar);
    sch.Run();
    if(pPat) {
      delete pPat;
    }
