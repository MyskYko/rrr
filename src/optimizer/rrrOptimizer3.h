...
namespace rrr {
  ...
  public:
    // constructors
    Optimizer3(Parameter const *pPar, std::function<double(Ntk *)> CostFunction);
  ...
  template <typename Ntk, typename Ana>
  struct Optimizer3<Ntk, Ana>::Stats {
    int nTriedFis = 0;
    int nAddedFis = 0;
    int nTried = 0;
    int nAdded = 0;
  ...
  template <typename Ntk, typename Ana>
  template <typename... Args>
  inline void Optimizer3<Ntk, Ana>::Print(int nVerboseLevel, Args... args) {
  ...
  template <typename Ntk, typename Ana>
  void Optimizer3<Ntk, Ana>::ActionCallback(Action const &action) {
    if(nVerbose > 4) {
  ...
  template <typename Ntk, typename Ana>
  inline void Optimizer3<Ntk, Ana>::MarkTfo(int id) {
    // includes id itself
    if(id == target) {
      return;
    }
  ...
  template <typename Ntk, typename Ana>
  inline bool Optimizer3<Ntk, Ana>::Timeout() {
    if(nTimeout) {
  ...
  template <typename Ntk, typename Ana>
  inline void Optimizer3<Ntk, Ana>::SetRandPiOrder() {
  ...
  template <typename Ntk, typename Ana>
  inline void Optimizer3<Ntk, Ana>::SetRandCosts() {
  ...
  template <typename Ntk, typename Ana>
  inline void Optimizer3<Ntk, Ana>::SortFanins(int id) {
    switch(nSortType) {
    case 0: // no sorting
      break;
  ...
  template <typename Ntk, typename Ana>
  inline void Optimizer3<Ntk, Ana>::SortFanins() {
  ...
  template <typename Ntk, typename Ana>
  inline bool Optimizer3<Ntk, Ana>::RemoveRedundantFanins(int id, bool fRemoveUnused) {
  ...
  template <typename Ntk, typename Ana>
  inline bool Optimizer3<Ntk, Ana>::RemoveRedundantFaninsRandom(int id, bool fRemoveUnused) {
    assert(pNtk->GetNumFanouts(id) > 0);
    bool fReduced = false;
  ...
  /*
  template <typename Ntk, typename Ana>
  inline bool Optimizer3<Ntk, Ana>::RemoveRedundantFaninsOneRandom(int id, bool fRemoveUnused) {
    assert(pNtk->GetNumFanouts(id) > 0);
    // generate random order
    vTmp.resize(pNtk->GetNumFanins(id));
    std::iota(vTmp.begin(), vTmp.end(), 0);
  ...
  */
  ...
  template <typename Ntk, typename Ana>
  bool Optimizer3<Ntk, Ana>::RemoveRedundancyOneTraversal(bool fSubRoutine) {
  ...
  /*
  template <typename Ntk, typename Ana>
  void Optimizer3<Ntk, Ana>::ReduceRandom() {
    pNtk->Sweep(false);
    std::vector<int> vInts = pNtk->GetInts();
  ...
  */
  ...
  template <typename Ntk, typename Ana>
  bool Optimizer3<Ntk, Ana>::RemoveRedundancy() {
    time_point timeStart = GetCurrentTime();
    bool fReduced = false;
  ...
  template <typename Ntk, typename Ana>
  bool Optimizer3<Ntk, Ana>::RemoveRedundancyLegacy() {
    time_point timeStart = GetCurrentTime();
    bool fReduced = false;
    std::vector<int> vInts = pNtk->GetInts();
  ...
  template <typename Ntk, typename Ana>
  bool Optimizer3<Ntk, Ana>::RemoveRedundancyRandom() {
    // TODO: compatible dc?
    time_point timeStart = GetCurrentTime();
    bool fReduced = false;
    //pNtk->Sweep(false);
  ...
  template <typename Ntk, typename Ana>
  bool Optimizer3<Ntk, Ana>::Reduce() {
  ...
  template <typename Ntk, typename Ana>
  template <typename T>
  T Optimizer3<Ntk, Ana>::SingleAdd(int id, T begin, T end) {
    time_point timeStart = GetCurrentTime();
    MarkTfo(id);
  ...
  template <typename Ntk, typename Ana>
  int Optimizer3<Ntk, Ana>::MultiAdd(int id, std::vector<int> const &vCands, int nMax) {
    time_point timeStart = GetCurrentTime();
    MarkTfo(id);
    pNtk->ForEachFanin(id, [&](int fi) {
      vTfoMarks[fi] = true;
  ...
  template <typename Ntk, typename Ana>
  void Optimizer3<Ntk, Ana>::SingleResub(int id, std::vector<int> const &vCands) {
  ...
  template <typename Ntk, typename Ana>
  void Optimizer3<Ntk, Ana>::MultiResub(int id, std::vector<int> const &vCands, int nMax) {
    // NOTE: this assumes trivial collapse/decompose does not change cost
    // let us assume the node is not trivially redundant for now
    assert(pNtk->GetNumFanouts(id) != 0);
    assert(pNtk->GetNumFanins(id) > 1);
  ...
  template <typename Ntk, typename Ana>
  bool Optimizer3<Ntk, Ana>::SingleResubStop(int id, std::vector<int> const &vCands) {
  ...
  template <typename Ntk, typename Ana>
  bool Optimizer3<Ntk, Ana>::MultiResubStop(int id, std::vector<int> const &vCands, int nMax) {
  ...
  template <typename Ntk, typename Ana>
  bool Optimizer3<Ntk, Ana>::MultiTargetResub(std::vector<int> vTargets, int nMax) {
    statsLocal.nTried++;
    // save
    int slot = pNtk->Save();
    double cost = CostFunction(pNtk);
  ...
  template <typename Ntk, typename Ana>
  void Optimizer3<Ntk, Ana>::ApplyReverseTopologically(std::function<void(int)> const &func) {
    std::vector<int> vInts = pNtk->GetInts();
    for(critr it = vInts.rbegin(); it != vInts.rend(); it++) {
      if(Timeout()) {
        break;
  ...
  template <typename Ntk, typename Ana>
  void Optimizer3<Ntk, Ana>::ApplyRandomlyStop(std::function<bool(int)> const &func) {
    std::vector<int> vInts = pNtk->GetInts();
    std::shuffle(vInts.begin(), vInts.end(), rng);
    for(citr it = vInts.begin(); it != vInts.end(); it++) {
      if(Timeout()) {
  ...
  template <typename Ntk, typename Ana>
  void Optimizer3<Ntk, Ana>::ApplyCombinationSampledStop(int k, int n, std::function<bool(std::vector<int> const &)> const &func) {
    std::vector<int> vInts = pNtk->GetInts();
    for(int i = 0; i < n; i++) {
      if(Timeout()) {
        break;
  ...
  template <typename Ntk, typename Ana>
  void Optimizer3<Ntk, Ana>::ApplyMultisetSampledStop(int k, int n, std::function<bool(std::vector<int> const &)> const &func) {
    std::vector<int> vInts = pNtk->GetInts();
    for(int i = 0; i < n; i++) {
      if(Timeout()) {
        break;
  ...
  template <typename Ntk, typename Ana>
  Optimizer3<Ntk, Ana>::Optimizer3(Parameter const *pPar, std::function<double(Ntk *)> CostFunction) :
    pNtk(NULL),
  ...
  template <typename Ntk, typename Ana>
  void Optimizer3<Ntk, Ana>::SetPrintLine(std::function<void(std::string)> const &PrintLine_) {
  ...
  template <typename Ntk, typename Ana>
  void Optimizer3<Ntk, Ana>::Run(int iSeed, seconds nTimeout_) {
    rng.seed(iSeed);
    vRandPiOrder.clear();
    vRandCosts.clear();
  ...
  template <typename Ntk, typename Ana>
  void Optimizer3<Ntk, Ana>::SetNumTargets(int nTargets_) {
    nTargets = nTargets_;
  }
  ...
  template <typename Ntk, typename Ana>
  void Optimizer3<Ntk, Ana>::ResetSummary() {
    stats.clear();
    ana.ResetSummary();
  }
  
  template <typename Ntk, typename Ana>
  summary<int> Optimizer3<Ntk, Ana>::GetStatsSummary() const {
    summary<int> v;
  ...
  template <typename Ntk, typename Ana>
  summary<double> Optimizer3<Ntk, Ana>::GetTimesSummary() const {
    summary<double> v;
    for(auto const &entry: stats) {
