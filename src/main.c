...
int main(int argc, char **argv) {
  ...
  while ( ( c = Extra_UtilGetopt( argc, argv, "XYNJKBDRWTCGVPOAQSabcdegvh" ) ) != EOF )
  {
      switch ( c )
      {
      case 'X':
...
usage:
      Abc_Print( -2, "usage: rrr [-XYNJKBDRWTCGVPOAQS num] [-abdegh]\n" );
