...
int main(int argc, char **argv) {
  ...
  while ( ( c = Extra_UtilGetopt( argc, argv, "XYZNJKLBDRWTCGVPOAQSFUEabcdegvh" ) ) != EOF )
  {
      switch ( c )
      {
      case 'X':
...
usage:
      Abc_Print( -2, "usage: rrr [-XYZNJKLBDRWTCGVPOAQS num] [-FU str] [-abdegh]\n" );
